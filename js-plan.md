...
| # | Topic | Decision |
|---|-------|----------|
| 1 | Repository | `presto-pay-sdk-js`. The wire contract (built from §3) and the test vectors live in `presto-pay-spec` and are vendored here as a plain copy at `spec/` — not a git submodule — because four SDKs now implement the same contract (§9) |
| 2 | Package | `@prestouniverse/presto-pay-sdk` on npm, **ESM only**. Node 22.12+ can `require()` it directly; Node 18.20–22.11 CommonJS callers use dynamic `import()` |
| 3 | Runtimes (0.1.0) | Node **18.20+** (18.20, 20, 22, 24, 26 in CI), Cloudflare Workers, Vercel Edge. Bun and Deno: best effort, not in the 0.1.0 support statement. Node 18's global `crypto` is main-thread only — unavailable inside `worker_threads` Workers and forked child processes — so a request handler needs Node 20+ to run there |
...
- `presto-pay-spec` is a repository of its own holding `wire-contract.md` — the reviewed, language-neutral
  version of §3 — along with `vectors/` and `keys/`. It is vendored here as a **plain copy** at `spec/`, not a
  git submodule: each SDK repo copies in the commit it needs and updates the copy by hand when the contract
  changes, rather than every clone and CI job needing submodule init/update. The Python, Go and PHP SDKs vendor
  their own copy of the same commit the same way; with the vectors living in this repo instead, the other three
  would either copy them and drift, or depend on an npm package to run their tests.
...
spec/                  vendored copy of presto-pay-spec (wire-contract.md, vectors/, keys/), not a submodule
...
|---|-----------|---------------|
| 0 | Contract | `presto-pay-spec` created, `wire-contract.md` written from §3 and reviewed, vendored here as a plain copy at `spec/`; vectors written for canonicalization, timestamps and signatures. Both §3.4 captures verify against Presto's certificate in `keys/` and go in as the first two canonical vectors; the only thing still to make is a throwaway keypair for `spec/keys` |
